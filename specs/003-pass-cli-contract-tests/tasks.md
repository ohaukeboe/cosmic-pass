...
- [ ] T001 Create `tests/support/mod.rs` declaring `pub mod real_cli;`, and an empty
...
- [ ] T002 Create `tests/pass_cli_contract.rs` containing only `mod support;` plus one
...
- [ ] T003 [P] Add `proton-pass-cli` to the devShell in `flake.nix`: in `devShells`, add
...
- [ ] T004 [P] Set `COSMIC_PASS_REQUIRE_CLI = "1"` in the same devShell attrset in `flake.nix`.
...
- [ ] T005 Implement `ValueKind` and `RealCli` in `tests/support/real_cli.rs` per
      [data-model.md](./data-model.md): `RealCli { path: PathBuf, version: CliVersion }`.
- [ ] T006 Implement `Resolution` in `tests/support/real_cli.rs` as the three-variant enum from
      data-model.md — `Found(RealCli)`, `Skip { looked_for: String }`,
      `Fail { reason: String }`. No other variant.
- [ ] T007 Implement `RealCli::find() -> &'static Resolution` in `tests/support/real_cli.rs`,
...
- [ ] T008 Implement the skip-versus-fail rule in `RealCli::find`:
      ...
      (research.md D5).
- [ ] T009 Implement the skip macro in `tests/support/real_cli.rs`: a
...
- [ ] T010 Implement `IsolatedEnv` in `tests/support/real_cli.rs` per data-model.md: owns a
...
- [ ] T011 Extend `IsolatedEnv` to neutralise the keyring reach (research.md D3, the real
...
- [ ] T012 Implement `IsolatedEnv::runner(&self, cli: &RealCli) -> TokioRunner` in
...
- [ ] T013 Implement `IsolatedEnv::wrote_only_inside_itself(&self) -> Result<(), Vec<PathBuf>>`
...
- [ ] T014 [US1] RED-FIRST. Add `isolation::probe_is_unauthenticated` to
...
- [ ] T015 [US1] Add `isolation::probe_writes_nothing_outside_its_temp_dir` to
      `tests/pass_cli_contract.rs` using `IsolatedEnv::wrote_only_inside_itself` from T013
      (guarantee 2).
- [ ] T016 [US1] Add `version::reported_version_is_at_or_above_tested_min` to
...
- [ ] T017 [P] [US1] Add `surface::top_level_commands_exist` to `tests/pass_cli_contract.rs`:
      `--help` exits 0 and its stdout contains `login`, `logout`, `info`, `vault`, `item`.
- [ ] T018 [P] [US1] Add `surface::info_accepts_output_json` to `tests/pass_cli_contract.rs`:
      `info --help` exits 0 and stdout contains `--output` and `json`.
- [ ] T019 [P] [US1] Add `surface::vault_list_accepts_output_json` to
      `tests/pass_cli_contract.rs`: `vault list --help` exits 0 and stdout contains `--output`
      and `json`.
- [ ] T020 [P] [US1] Add `surface::item_list_accepts_share_id_output_and_show_secrets` to
      `tests/pass_cli_contract.rs`: `item list --help` exits 0 and stdout contains `--share-id`,
      `--output`, `--show-secrets`.
- [ ] T021 [P] [US1] Add `surface::item_view_accepts_share_id_item_id_and_field` to
      `tests/pass_cli_contract.rs`: `item view --help` exits 0 and stdout contains `--share-id`,
      `--item-id`, `--field`.
- [ ] T022 [P] [US1] Add `surface::item_totp_accepts_share_id_item_id_and_output` to
...
- [ ] T023 [US1] Add `argv::every_command_the_app_builds_is_accepted` to
      ...
      (research.md D4).
- [ ] T024 [US1] RED-FIRST. Add `argv::ids_must_use_the_equals_form` to
...
- [ ] T025 [US1] Add `classify::unauthenticated_stderr_is_signed_out` to
...
- [ ] T026 [US1] Add `env::stdout_carries_payload_only` to `tests/pass_cli_contract.rs`
...
- [ ] T027 [US2] Add the double gate to every test in `tests/pass_cli_live.rs`:
...
- [ ] T028 [US2] Add `preflight::session_exists` to `tests/pass_cli_live.rs` (FR-018): run
...
- [ ] T029 [US2] Add `just test-live` to `justfile`: `COSMIC_PASS_LIVE=1 cargo nextest run
...
- [ ] T030 [US2] Implement `LiveScenario` in `tests/support/real_cli.rs` per data-model.md:
...
- [ ] T031 [US2] Implement the coverage report in `tests/pass_cli_live.rs`: one line per
...
- [ ] T032 [P] [US2] Add `live::info_parses` to `tests/pass_cli_live.rs`: `info --output json`
      through `pass::parse::parse_account`; assert the `AccountId` is non-empty. Never print it.
- [ ] T033 [P] [US2] Add `live::vault_list_parses` to `tests/pass_cli_live.rs`:
      `vault list --output json` through `parse_vaults`; assert at least one vault and that
      every `share_id` and `vault_id` is non-empty.
- [ ] T034 [P] [US2] Add `live::item_list_parses_with_secrets` to `tests/pass_cli_live.rs`:
...
- [ ] T035 [P] [US2] Add `live::item_list_parses_plain` to `tests/pass_cli_live.rs`: the same
...
- [ ] T036 [US2] Add `live::field_view_parses` to `tests/pass_cli_live.rs`: find an item whose
      ...
      account → `NotCovered`.
- [ ] T037 [US2] Add `live::totp_parses` to `tests/pass_cli_live.rs`: find an item with a TOTP
...
- [ ] T038 [US2] Add `live::field_inside_a_section_parses` to `tests/pass_cli_live.rs`
...
- [ ] T039 [P] [US2] Add `live::malformed_share_id_is_not_found` to `tests/pass_cli_live.rs`:
...
- [ ] T040 [P] [US2] Add `live::missing_field_is_field_missing` to `tests/pass_cli_live.rs`:
...
- [ ] T041 [US2] Add `live::latency_is_reported` to `tests/pass_cli_live.rs`: time `info`,
...
- [ ] T042 [US3] Implement `FixtureShape` in `tests/support/real_cli.rs` per data-model.md:
...
- [ ] T043 [US3] RED-FIRST. Add unit tests for `FixtureShape` derivation in
...
- [ ] T044 [US3] Add `shape::committed_fixtures_still_match_reality` to
...
- [ ] T045 [US3] Report shape differences as two sorted lists — paths only in fresh output
...
- [ ] T046 [P] Correct `specs/001-quick-access-launcher/contracts/pass-cli.md`, amendment 1
      ...
      logging on 2.3.3.
- [ ] T047 [P] Correct `specs/001-quick-access-launcher/contracts/pass-cli.md`, amendment 2:
...
- [ ] T048 Update the coverage table in
      ...
      reason.
- [ ] T049 Extend `scripts/leak-scan.sh` with a live step (FR-020). The existing scan greps for
...
- [ ] T050 [P] Document the live suite in `README.md`, `CLAUDE.md` and `AGENTS.md` under
      ...
      across both.
- [ ] T051 Run `just check` inside `nix develop`. Confirm green, confirm no
...
- [ ] T052 Run quickstart V1–V10 from [quickstart.md](./quickstart.md) and record the outcome of
      each, including which live scenarios reported `not covered` on the account used.
- [ ] T053 Demonstrate SC-002 (quickstart V4): temporarily apply each of the three mutations to
