...
- [x] T083 Run quickstart.md V1–V9 on a real COSMIC session — done 2026-09-18 by driving the live session synthetically; results and the four defects found are recorded in research.md "Live validation (2026-09-18)". Three checks remain open and need a human: the browser sign-in flow (V5.2), focus-loss hiding (V2.8), and a genuinely locked keyring (V7.2); they are tracked as research items V4, V6 and V7 and as `bd` issues. Original task text: Run quickstart.md V1–V9 on a real COSMIC session (**needs the user**). Record results and any contract corrections (e.g. locked-session error text, whether killing the helper clears the selection) in `specs/001-quick-access-launcher/research.md` "Open validation items", and file `bd` issues for failures.
...
- [x] T085 Bold the matched characters of result titles in `src/app/view/list.rs` using the `title_indices` already returned by `src/core/search.rs` (rich text spans) per tasks T033 / FR-008 (partial)
- [x] T086 Offer every website of an item in the action list: keep all URLs as fields in `src/pass/parse.rs` (`url`, `url2`, … or one entry per URL) and list them in `all_actions` in `src/core/actions.rs` per spec Edge Cases "Item has multiple websites or multiple TOTP fields" (partial)
- [x] T087 Report a network failure distinctly: track the last refresh error in `src/core/state.rs` and show "Can't reach Proton Pass — showing saved items" in the stale indicator in `src/app/view/list.rs`, with a reducer test per FR-020 (partial)
...
- [x] T089 Measure window-open latency for SC-001: log the elapsed time from `Msg::Toggle`/`dbus_activation` to the layer surface being ready in `src/app/mod.rs` at debug level, and record the measured p95 in `specs/001-quick-access-launcher/research.md` during quickstart V2 per SC-001 (missing)
- [x] T090 Derive the popup top margin from the active output height (~20%) instead of the fixed `TOP_MARGIN` in `src/app/surface.rs`, or amend plan R2 to state the fixed offset per plan R2 / tasks T031 (partial)
...
- [X] T092 Hide the popup when the clipboard helper reports ownership instead of on submit: move the `hide()` out of `finish_copy` in `src/core/state.rs` into the `Msg::CopyFinished` arm, with a reducer test, so a paste immediately after Enter cannot read an empty selection per US1/AC3 / SC-003 (partial)
- [ ] T093 Account for the item-count difference seen live (`pass-cli` 616, app 572): confirm in `src/pass/parse.rs` that every dropped item is `state == "trashed"`, and add a parser test covering the other states `pass-cli` can report per FR-010 / FR-005 (partial)
- [ ] T094 Verify section-field copy for SSH, Wi-Fi and custom items against a real `pass-cli` item that has sections, and adjust the `section_fields` naming in `src/pass/parse.rs` if `item view --field=` needs a qualified `Section.field` form per FR-011 (partial)
- [X] T095 Strengthen the selection indicator in `src/app/view/list.rs` and `src/app/view/actions.rs`: add an accent border or text tint so the highlighted row clears the 3:1 contrast guideline for non-text indicators instead of the measured 1.9:1 per FR-004 / US1/AC2 (partial)
- [X] T096 Render a placeholder instead of a blank value line for custom fields of variant `Text` in `src/app/view/actions.rs` and `src/app/view/detail.rs`, since the cache deliberately drops their values per FR-013 (partial)
- [ ] T097 Run the three live checks that need a human and record the outcome in `specs/001-quick-access-launcher/research.md` V4/V6/V7: browser sign-in from the popup's Sign in button, focus loss hiding the popup, and a genuinely locked keyring per FR-021 / FR-003 / FR-024a (partial) — **needs the user**
- [X] T098 Make the action-list `Enter` caption follow the highlighted row rather than the primary-field row in `src/app/view/actions.rs`, so the visible chord names the entry Enter will copy per FR-013 (partial)
- [X] T099 Give the TOTP progress bar an explicit width in `src/app/view/detail.rs` so it spans the value column instead of the ~100 px left-aligned bar per FR-019 (partial)
- [X] T100 Reconcile the clipboard test layout with the plan: either move the helper-lifecycle tests from `src/clipboard/mod.rs` into `tests/clipboard_integration.rs` as `plan.md` names, or amend the plan's test list to record that they live as unit tests against `tests/fixtures/fake-clipboard-serve` per plan: test layout (partial)
- [X] T101 De-duplicate the repeated "Beads Issue Tracker" section in `AGENTS.md` and correct the stale `.beads/dolt/` path to `.beads/embeddeddolt/`, then re-sync `AGENTS.md` with `CLAUDE.md` per Constitution V (partial)

---

## Phase 11: Convergence (second pass)

**Purpose**: Close gaps found by a deeper `/speckit-converge` sweep of the same artifacts.

- [X] T102 Stop destroying a readable cache when the keyring is merely locked: in `src/cache/store.rs` `load`, return `None` without deleting the file when `keys.key(false)` yields `None` (locked keyring, `oo7` error, or `COSMIC_PASS_NO_KEYRING=1`), and delete only on decrypt/decode/version failure, with a `tests/cache_integration.rs` case that writes a cache, locks the key store, reloads, and asserts the file survives per FR-024a / `contracts/cache-format.md` rules table (contradicts)
- [X] T103 Make the Preferences editor operable from the keyboard: map chords in `src/app/keys.rs` (which currently returns `None` for every key in `Mode::Preferences`) to `Msg::AdjustClipboardClear`, `Msg::StartRebind` and `Msg::ResetShortcuts`, since those messages are produced only by mouse buttons in `src/app/view/preferences.rs` and `core.set_keyboard_nav(false)` in `src/app/mod.rs` disables Tab traversal per FR-004 (partial)
- [X] T104 Give the status panel keyboard actions: add chords reaching `Msg::StartLogin` and `Msg::Startup` in `src/app/keys.rs` and `src/config.rs` `Action`, because "Sign in" and "Try again" in `src/app/view/status.rs` are mouse-only and keyboard navigation is disabled, so a signed-out user cannot start sign-in without a mouse per FR-004 / FR-021 (partial)
- [X] T105 Allow revealing every secret field, not just the primary one: `toggle_reveal` in `src/core/state.rs` resolves the field through `primary_action` and `src/app/view/detail.rs` substitutes the revealed value only when the field name equals the primary, so a card's verification number and PIN and a login's note stay permanently masked per FR-018 (partial)
- [X] T106 Stop claiming saved items are on screen when there are none: gate the `Can't reach Proton Pass — showing saved items` branch of `stale_notice` in `src/core/state.rs` on `!self.data.items.is_empty()`, since a cold start with no cache and no network renders it directly above `No items found` in `src/app/view/list.rs`, with a reducer test per FR-020 / US4/AC3 (partial)
- [X] T107 Show the stale indicator outside the result list: `model.stale_notice()` has a single view call site in `src/app/view/list.rs`, so `src/app/view/detail.rs` and `src/app/view/actions.rs` give no hint that the data may be out of date per FR-022 (partial)
- [X] T108 Expose the rest of the identity fields: `IdentityBody` in `src/pass/parse.rs` keeps only five members and, unlike `WifiBody` and `SshBody`, has no `sections` member, so most identity fields and all identity custom fields never become `FieldRef`s for the action list per FR-013 / FR-011 (partial)
- [ ] T109 Decide and record what copying a cached non-secret field must do when the item is gone: `start_copy` in `src/core/state.rs` returns a stored `field.value` with no backend call — asserted deliberately by `copy_url_needs_no_backend_call` in `tests/story2_other_fields.rs` — so a deleted item yields a stale copy and never shows "Item no longer exists"; either re-fetch or amend the spec edge case per spec Edge Cases "Item was deleted or changed in Proton Pass" (contradicts)
- [X] T110 Make `scripts/leak-scan.sh` fail loudly instead of passing vacuously: it prints "no leaks found" even when the app run was skipped for want of a Wayland session, and `COSMIC_PASS_NO_KEYRING=1` means no cache file is ever written, so the encrypted cache is never among the scanned files per SC-006 (partial)
- [X] T111 Stop the hidden result list from moving while Preferences is open: in `src/app/keys.rs` the `ArrowDown`/`ArrowUp`/`PageDown`/`PageUp` arms run before the `Mode::Preferences` early return, emitting `Msg::SelectNext`/`Msg::SelectPrev` for a list the user cannot see, which `contracts/keyboard.md` does not define per FR-004 / `contracts/keyboard.md` (contradicts)
- [X] T112 Reconcile the window placement wording: `spec.md` FR-002 still says the window opens "centered on the active display", while `src/app/surface.rs` anchors to `Anchor::TOP` with the fixed `TOP_MARGIN` spacer rendered in `src/app/view/mod.rs`; `research.md` R2 records the amendment but the spec was never updated — correct the code or raise the divergence with the maintainer per FR-002 (partial)
- [X] T113 Record the `--show-secrets` deviation against FR-014 in `plan.md` Complexity Tracking: `src/pass/backend.rs` passes `--show-secrets` on every background refresh, so all secrets enter the process on refresh rather than only on user request, mitigated by the `Zeroizing` stdout in `src/pass/runner.rs` and the stripping parser in `src/pass/parse.rs` and explained in `research.md`, but not listed as a tracked deviation per FR-014 (partial)
- [X] T114 Close the clipboard gap left when the app dies: the helper's own watchdog in `src/clipboard/serve.rs` fires at `timeout + 5` s, so an orphaned secret outlives the configured timeout by 5 s rather than the 1 s SC-005 allows per SC-005 (partial)
- [X] T115 Render the busy indicator in the action list: the pending spinner exists only in `src/app/view/list.rs`, and a copy started from the action list keeps `Mode::Actions`, whose view in `src/app/view/actions.rs` shows no pending state per spec Edge Cases "Retrieving a secret takes longer than expected" (partial)

---

## Phase 12: Defects found while implementing Phases 10-11

**Purpose**: Record work that adversarial review surfaced during `/speckit-implement`, not by
assessing the artifacts. All are closed; they are listed so the requirements they touch have an
honest history.

- [X] T116 Bind a revealed secret to the fetch that produced it: `Msg::RevealFetched` was guarded only by `(key, index)`, so a result issued for one field was accepted for whichever field later occupied that index. A monotonic generation is now stamped into `Effect::FetchReveal`, echoed back, and compared, and the reveal has its own cancellation scope separate from the TOTP fetch per FR-018 / FR-019 (regression from T105)
- [X] T117 Invalidate a revealed secret when the item it came from changes: `replace_items` swapped `data.items` wholesale without clearing `revealed`, so a refresh that reshaped the field list re-labelled a plaintext already on screen. The reveal is now pinned to the item, position, field and `modified_at`, and dropped only when that no longer holds, so an unchanged refresh still does not re-mask per FR-018 / FR-025 (regression from T105)
- [X] T118 Clear the revealed secret and the one-time code on sign-out and account switch: `clear_data` left them in place per FR-025 / FR-003 (regression from T105)
- [X] T119 Stop the reveal cycle sticking on a fetch error: the `Err` arm reset the cycle to the first secret, so a field that always fails made every later secret unreachable per FR-018 (regression from T105)
- [X] T120 Catch an account switch from every prior session state: `signed_in` inferred a switch from `session`/`cache_account`, so a switch reached through `Locked`, `Unknown`, `Error` or `Checking` left the previous account's plaintext on screen and skipped the cache delete. Now tracked by the account the loaded data belongs to, which a listing landing after a lock no longer erases per FR-024b / FR-025 (bd cosmic-pass-wqx.44)
- [X] T121 Render `view.notice` in the detail and action panes: only `list.rs` read it, so a failed reveal showed the user nothing and the action list could not show why a copy did nothing — including the "no one-time code" message per FR-020 / US2/AC4 (bd cosmic-pass-wqx.43)
- [X] T122 Route `Mode::Preferences` to `preferences::view`: `popup()` sent it to `list::view`, so the preferences editor was never rendered and `preferences::view` was dead code, leaving FR-027 unmet and T103's chords driving an invisible pane per FR-027 / Constitution I (bd cosmic-pass-wqx.42)
