...
- [X] T109 Decided: re-fetch. `start_copy` in `src/core/state.rs` no longer short-circuits on a stored `field.value`, so every field is read through `Effect::FetchAndCopy` and a deleted item reaches the "Item no longer exists" notice and refresh the spec's Edge Cases require. The spec needed no amendment — the code had diverged from it. `copy_url_needs_no_backend_call` is replaced by `copy_url_fetches_the_current_value` in `tests/story2_other_fields.rs` (cosmic-pass-wqx.34)
