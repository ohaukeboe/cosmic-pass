...
- [ ] T092 Hide the popup when the clipboard helper reports ownership instead of on submit: move the `hide()` out of `finish_copy` in `src/core/state.rs` into the `Msg::CopyFinished` arm, with a reducer test, so a paste immediately after Enter cannot read an empty selection per US1/AC3 / SC-003 (partial)
...
- [ ] T095 Strengthen the selection indicator in `src/app/view/list.rs` and `src/app/view/actions.rs`: add an accent border or text tint so the highlighted row clears the 3:1 contrast guideline for non-text indicators instead of the measured 1.9:1 per FR-004 / US1/AC2 (partial)
- [ ] T096 Render a placeholder instead of a blank value line for custom fields of variant `Text` in `src/app/view/actions.rs` and `src/app/view/detail.rs`, since the cache deliberately drops their values per FR-013 (partial)
...
- [ ] T098 Make the action-list `Enter` caption follow the highlighted row rather than the primary-field row in `src/app/view/actions.rs`, so the visible chord names the entry Enter will copy per FR-013 (partial)
- [ ] T099 Give the TOTP progress bar an explicit width in `src/app/view/detail.rs` so it spans the value column instead of the ~100 px left-aligned bar per FR-019 (partial)
- [ ] T100 Reconcile the clipboard test layout with the plan: either move the helper-lifecycle tests from `src/clipboard/mod.rs` into `tests/clipboard_integration.rs` as `plan.md` names, or amend the plan's test list to record that they live as unit tests against `tests/fixtures/fake-clipboard-serve` per plan: test layout (partial)
- [ ] T101 De-duplicate the repeated "Beads Issue Tracker" section in `AGENTS.md` and correct the stale `.beads/dolt/` path to `.beads/embeddeddolt/`, then re-sync `AGENTS.md` with `CLAUDE.md` per Constitution V (partial)
...
- [ ] T102 Stop destroying a readable cache when the keyring is merely locked: in `src/cache/store.rs` `load`, return `None` without deleting the file when `keys.key(false)` yields `None` (locked keyring, `oo7` error, or `COSMIC_PASS_NO_KEYRING=1`), and delete only on decrypt/decode/version failure, with a `tests/cache_integration.rs` case that writes a cache, locks the key store, reloads, and asserts the file survives per FR-024a / `contracts/cache-format.md` rules table (contradicts)
- [ ] T103 Make the Preferences editor operable from the keyboard: map chords in `src/app/keys.rs` (which currently returns `None` for every key in `Mode::Preferences`) to `Msg::AdjustClipboardClear`, `Msg::StartRebind` and `Msg::ResetShortcuts`, since those messages are produced only by mouse buttons in `src/app/view/preferences.rs` and `core.set_keyboard_nav(false)` in `src/app/mod.rs` disables Tab traversal per FR-004 (partial)
- [ ] T104 Give the status panel keyboard actions: add chords reaching `Msg::StartLogin` and `Msg::Startup` in `src/app/keys.rs` and `src/config.rs` `Action`, because "Sign in" and "Try again" in `src/app/view/status.rs` are mouse-only and keyboard navigation is disabled, so a signed-out user cannot start sign-in without a mouse per FR-004 / FR-021 (partial)
- [ ] T105 Allow revealing every secret field, not just the primary one: `toggle_reveal` in `src/core/state.rs` resolves the field through `primary_action` and `src/app/view/detail.rs` substitutes the revealed value only when the field name equals the primary, so a card's verification number and PIN and a login's note stay permanently masked per FR-018 (partial)
- [ ] T106 Stop claiming saved items are on screen when there are none: gate the `Can't reach Proton Pass — showing saved items` branch of `stale_notice` in `src/core/state.rs` on `!self.data.items.is_empty()`, since a cold start with no cache and no network renders it directly above `No items found` in `src/app/view/list.rs`, with a reducer test per FR-020 / US4/AC3 (partial)
- [ ] T107 Show the stale indicator outside the result list: `model.stale_notice()` has a single view call site in `src/app/view/list.rs`, so `src/app/view/detail.rs` and `src/app/view/actions.rs` give no hint that the data may be out of date per FR-022 (partial)
- [ ] T108 Expose the rest of the identity fields: `IdentityBody` in `src/pass/parse.rs` keeps only five members and, unlike `WifiBody` and `SshBody`, has no `sections` member, so most identity fields and all identity custom fields never become `FieldRef`s for the action list per FR-013 / FR-011 (partial)
...
- [ ] T110 Make `scripts/leak-scan.sh` fail loudly instead of passing vacuously: it prints "no leaks found" even when the app run was skipped for want of a Wayland session, and `COSMIC_PASS_NO_KEYRING=1` means no cache file is ever written, so the encrypted cache is never among the scanned files per SC-006 (partial)
- [ ] T111 Stop the hidden result list from moving while Preferences is open: in `src/app/keys.rs` the `ArrowDown`/`ArrowUp`/`PageDown`/`PageUp` arms run before the `Mode::Preferences` early return, emitting `Msg::SelectNext`/`Msg::SelectPrev` for a list the user cannot see, which `contracts/keyboard.md` does not define per FR-004 / `contracts/keyboard.md` (contradicts)
- [ ] T112 Reconcile the window placement wording: `spec.md` FR-002 still says the window opens "centered on the active display", while `src/app/surface.rs` anchors to `Anchor::TOP` with the fixed `TOP_MARGIN` spacer rendered in `src/app/view/mod.rs`; `research.md` R2 records the amendment but the spec was never updated — correct the code or raise the divergence with the maintainer per FR-002 (partial)
- [ ] T113 Record the `--show-secrets` deviation against FR-014 in `plan.md` Complexity Tracking: `src/pass/backend.rs` passes `--show-secrets` on every background refresh, so all secrets enter the process on refresh rather than only on user request, mitigated by the `Zeroizing` stdout in `src/pass/runner.rs` and the stripping parser in `src/pass/parse.rs` and explained in `research.md`, but not listed as a tracked deviation per FR-014 (partial)
- [ ] T114 Close the clipboard gap left when the app dies: the helper's own watchdog in `src/clipboard/serve.rs` fires at `timeout + 5` s, so an orphaned secret outlives the configured timeout by 5 s rather than the 1 s SC-005 allows per SC-005 (partial)
- [ ] T115 Render the busy indicator in the action list: the pending spinner exists only in `src/app/view/list.rs`, and a copy started from the action list keeps `Mode::Actions`, whose view in `src/app/view/actions.rs` shows no pending state per spec Edge Cases "Retrieving a secret takes longer than expected" (partial)
